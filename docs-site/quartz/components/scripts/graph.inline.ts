...
async function renderGraph(graph: HTMLElement, fullSlug: FullSlug) {
  ...
  type LabelBox = { text: string; x: number; y: number; width: number; height: number }
  ...
  // Re-fitting is cheap but not free, so while the layout is still moving it happens
  // on a slow cadence rather than every frame.
  ...
  function resolveLabelOverlaps() {
    ...
    // the captions actually drawn, in graph coordinates, for the overlap check
    const boxes: LabelBox[] = kept.map((item) => ({
